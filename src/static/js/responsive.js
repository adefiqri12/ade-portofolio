...
document.addEventListener("DOMContentLoaded", () => {
	...
	prevBtn.addEventListener("click", () => {
		...
		setTimeout(() => {
			currentStartIndex =
				(currentStartIndex - cardsPerPage + totalProjects) % totalProjects;
			updateVisibility();
			projectsGrid.style.transform = "translateX(-20px)";
	...
	function updateVisibility() {
		projectCards.forEach((card, index) => {
			...
			const shiftedIndex = (index - currentStartIndex + totalProjects) % totalProjects;
...
function adjustChatPanel() {
	...
	if (window.innerWidth < 768) {
		...
		const paddingRight = Number.parseFloat(mainStyles.paddingRight) || 0;
